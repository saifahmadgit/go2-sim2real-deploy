...
CKPT_PATH = os.path.join(SCRIPT_DIR, "model_22600.pt")
...
# This is BOTH:
# 1) the stand pose we want to reach before policy
# 2) the "default pose" used in obs (q-default) and action->target conversion
...
def build_obs(raw, command_3, last_action_12):
    gyro = torch.tensor(raw["imu"]["gyro_rad_s"], dtype=torch.float32)  # rad/s
    proj_g = projected_gravity_from_quat_body_in_world(
        raw["imu"]["quat_wxyz"]
    )  # unit vector
...
def make_command_list():
    return [COMMAND_VX, COMMAND_VY, COMMAND_WZ]
...
# robot_run:
# release -> go to stand -> prompt -> policy
...
def run_robot_run(policy):
    ...
    # 1) Release high-level first (as requested)
    ...
    # 3) Ramp to stand pose (absolute angles)
    ...
    print("Stand pose commanded.")

    # 4) Prompt to start policy
    print("\n*** READY TO START POLICY ***")
    print(f"Type {ARM_TEXT} to start running the policy. Anything else aborts.")
    user = input("> ").strip()
    if user != ARM_TEXT:
        print("Aborted. Standing hold continues until you stop the script.")
        return

    # switch gains to policy gains
    shared["kp"] = float(POLICY_KP)
    shared["kd"] = float(POLICY_KD)

    print(f"Starting policy loop at {POLICY_HZ} Hz.")

    # 5) Policy loop (50Hz) updates shared target
    dt = 1.0 / POLICY_HZ
    step = 0
    ...
    try:
        ...
        print("\nCTRL+C received. Sending safe stop packets...")
        for _ in range(200):
            for i in range(12):
                low_cmd.motor_cmd[i].q = go2.PosStopF
                low_cmd.motor_cmd[i].dq = go2.VelStopF
